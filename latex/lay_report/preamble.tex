...
\usepackage[left=62mm,right=28mm,top=20mm,bottom=18mm, heightrounded, marginparwidth=43mm, marginparsep=8mm]{geometry} % Use nice margins. Does give a small change in the default page margins. 
...
\fancyhead[RE]{%
    \begin{adjustbox}{left, minipage=0.9\paperwidth}
        ...
        \fill[yellow!60,path fading=west] (0.6\linewidth,3em) rectangle (\linewidth,1.5em);
...
\fancyhead[LO]{%
    \begin{adjustbox}{right, minipage=0.9\paperwidth}
        ...
        \fill[yellow!60,path fading=east] (0,3em) rectangle (0.4\linewidth,1.5em);
        \end{tikzpicture}\end{adjustbox}}
\newcommand{\textoverline}[1]{$\overline{\mbox{#1}}$}
% The footers are specified but are not the same margin widths as the headers. 
\fancyfoot[LE]{\hspace*{-0.5\innermarginonesided}\begin{adjustbox}{left, minipage=0.9\paperwidth}
        \raggedright
...
\fancyfoot[RO]{\hspace*{0.5\innermarginonesided}\begin{adjustbox}{right, minipage=0.9\paperwidth}
        \raggedleft
...
\NewDocumentEnvironment{mysidenote}{}{% verticle offset
    \noindent
    \begin{minipage}[t]{\linewidth}		\begin{mdframed}[roundcorner=5pt, linecolor=oxfordblue, linewidth=2pt, backgroundcolor=yellow!60, shadow=true,shadowcolor=black,shadowsize=6pt]%
        }
        {%
        \end{mdframed}
    \end{minipage}
